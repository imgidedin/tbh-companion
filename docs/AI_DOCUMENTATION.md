...
| Auto-open Chests | `ProcessAutoOpenChests`, `SendSpaceBarToGameWindow`, `SaveHasOpenAllChestsSpaceUpgrade`, `SaveInventoryIsFull` | Feature opt-in que usa `GetBoxLog` vivo como gatilho, exige `runeLevels["1055"] > 0` (`OpenAllTypeChestAllAtOnce`) e envia SPACE ao jogo ate `BoxOpenLog` ou limite curto de tentativas. O `GetBoxLog` e montado pela classe IL2CPP antes do regex textual, para nao depender de texto/localizacao. Por padrao bloqueia o envio quando o inventario esta cheio. |
...
5. `RefreshLiveSaveCache` tenta ler o snapshot vivo do save em `bal` a cada ~500ms; se validar, promove `saveSource=memory`. Se o jogo estiver aberto e a falha for apenas inventario transitorio, nao promove ES3 fisico antigo.
...
10. Se `app.auto_open_chests=1`, `ProcessAutoOpenChests` examina apenas eventos novos desde o ultimo scan; `GetBoxLog` (`category=chest` sem `itemKey`) enfileira abertura pendente e `BoxOpenLog` (`itemKey>0`) confirma que um bau foi aberto. O worker primeiro exige o upgrade `OpenAllTypeChestAllAtOnce` (`runeLevels["1055"] > 0`); se o upgrade nao estiver no save ativo, ele desativa e persiste `app.auto_open_chests=0`. Enquanto pendente, se `app.auto_open_chests_block_full_inventory=1` e o tab `inventory` do save ativo tiver `emptySlots <= 0`, o envio fica pausado. Caso contrario, envia SPACE ao `TaskBarHero.exe` via `SendInput`, com throttle e limite curto de tentativas, deixando o proprio jogo abrir os baus pela mecanica oficial do upgrade.
...
- `app.auto_open_chests_block_full_inventory`: opcional; `1` por padrao. Quando `1`, `Auto-open Chests` nao envia SPACE se o inventario principal do save ativo estiver cheio (`inventory.tabs[id=inventory].emptySlots <= 0`). A pendencia permanece pausada para abrir depois que houver slot livre.
...
### Postura observacional e automacoes opt-in

O companion continua visual/observacional por padrao em relacao ao jogo. A unica excecao ativa permitida hoje e `Auto-open Chests`, que precisa estar explicitamente ligada pelo usuario.

Regras:

- nao simular mouse, teclado ou comandos de UI do jogo fora das excecoes opt-in documentadas;
...
- `Auto-open Chests` nao injeta codigo, nao escreve memoria e nao chama IL2CPP; quando o `LogManager` vivo registra um `GetBoxLog` novo, ele so atua se o save ativo tiver o upgrade oficial que abre todos os baus com Espaço (`OpenAllTypeChestAllAtOnce`, rune `1055`). Nesse caso usa `SendInput` para enviar SPACE ao jogo, repetindo com throttle ate `BoxOpenLog` confirmar abertura ou o limite de tentativas encerrar o pedido. Se `app.auto_open_chests_block_full_inventory=1`, o envio fica bloqueado enquanto o inventario principal estiver cheio;
...
- O menu da bandeja tambem mostra `Bloquear se inventário cheio`, persistido em `app.auto_open_chests_block_full_inventory`.
...
- `PlayerSaveData.commonSaveData` e listas de currency, herois, atributos, pets, runas, inventario, storage, trade stash, itens e aggregates; em 1.00.21 o campo do trade stash passou para `remakeTradingStashSaveDatas`/`List<RemakeTradingStashSaveData>`, mantendo o layout de slot, entao o refresh deve aceitar os dois nomes/tipos e o resumo deve continuar publicando a aba como `tradingStashSaveDatas`
...
Importante: `ReadLiveSaveSummary` le o modelo de save vivo (`bal.bgaw/bgax`) para estrutura, mas gold/EXP, pontos de habilidade e o stage recem-entrado podem ficar parados ali ate o jogo comitar autosave. Por isso, antes de chamar `BuildSaveSummaryJson`, o agente aplica dados quentes de runtime sobre o root vivo: `MonsterSpawnManager.MonsterList`/`SummonedMonsterList` atualiza `CommonSaveData.currentStageKey` pelo stage carregado nos monstros vivos, `vb.tp/vb.tq` atualiza `CurrencySaveData.Quantity` do gold e `vb.tz`/`vd` atualiza `HeroSaveData.HeroLevel`, `AbilityPoint`, `AllocatedHeroAbilityPoint` e `HeroExp` por `heroKey`. Quando um slot de inventario/storage/trade ja referencia `ItemUniqueId` mas `PlayerSaveData.itemSaveDatas` ainda nao trouxe o item no mesmo instante, o leitor vivo consulta o cache backend `wk` (`List<InventoryItemData>` em builds antigos; `Dictionary<string, InventoryItemData>` em 1.00.21): `InventoryItemData.UniqueKey` resolve o `ItemUniqueId` e `InventoryItemData.ItemID` resolve o `ItemKey`. O agente adiciona um `ItemSaveData` runtime minimo somente para IDs ja referenciados por slot/equipamento e ainda ausentes em `itemSaveDatas + cache`; assim o frontend recebe nome/imagem imediatamente sem esperar autosave fisico. Se ainda faltar item depois dessa fusao, o resumo vivo deve continuar sendo publicado com o slot marcado como `itemPending`/`missing-item-save` e `unresolvedSlots`; item pendente nao deve derrubar snapshots metricos de Ouro/EXP para o fallback ES3. O ES3 continua sendo fallback inteiro quando a leitura viva nao esta disponivel ou quando o jogo fecha. Use `--memory-stage-scan` para comparar managers runtime contra o save vivo: ele amostra `runtimeStageKey`, `DeadMonsterUnit -> Monster.cache -> MonsterInfoData`, `runtimeGold`, `runtimeHeroMetrics`, `saveGold` e `savePartyHeroLevels`.

Regra para `ObscuredInt`/`ObscuredLong`/`ObscuredFloat` em 1.00.21: o valor autoritativo nao e XOR simples. `ObscuredInt` e `ObscuredLong` decriptam como `(hiddenValue - currentCryptoKey) ^ currentCryptoKey`, com overflow unsigned natural do tamanho do campo. `ObscuredFloat` primeiro aplica `ACTkByte4.yro` ao `hiddenValue` (troca dos bytes 1 e 2), depois faz XOR com `currentCryptoKey` e interpreta os bits resultantes como `float`. Nao usar `fakeValue` para publicar Gold/EXP/level, porque ele pode ficar zerado ou conter lixo depois de update do jogo. Antes de aplicar runtime sobre o save vivo, validar sanidade: Gold deve ser finito, nao negativo e dentro do inteiro seguro do JSON/JS; level deve ser positivo e razoavel; EXP e pontos devem ser finitos e nao negativos; level/EXP runtime nao deve reduzir o valor ja presente no root vivo/ES3, exceto EXP menor quando o `runtime.level` do mesmo heroi e maior que o level salvo, porque o jogo pode reiniciar a barra de EXP no level-up antes do autosave. Trate a linha inteira de `runtimeHeroMetrics` como nao confiavel quando o `level` runtime do mesmo heroi falhar validacao; nesse caso nao aplique EXP nem pontos, porque offsets/decode quebrados podem produzir EXP numericamente plausivel mas oscilante. Se a metrica runtime falhar essa validacao, preserve o valor do root vivo/ES3 em vez de sobrescrever com zero ou numero absurdo.
...
Se a leitura viva falhar antes de qualquer leitura viva valida nesta sessao, `RefreshLiveSaveCache` marca `has_live_save=false` e grava `saveFallbackReason`. Se o erro for inventario transitorio com o jogo aberto, o worker nao promove o ultimo `file_save` lido por ES3; ele aguarda um snapshot vivo consistente. Nao misturar blocos de memoria e arquivo no mesmo payload.

Se `ReadLiveSaveSummary` ja teve pelo menos uma leitura valida e uma leitura viva posterior falhar de forma transitoria, o agente deve manter o ultimo save ativo vindo de memoria em vez de promover imediatamente o ES3. Isso evita regressao temporaria de `currentStageKey`, gold, EXP e inventario para um autosave antigo enquanto o processo ainda esta aberto. O ES3 continua sendo fallback quando a leitura viva nao esta disponivel por motivo nao transitorio ou quando o jogo fecha.
...
- inventario, storage e stash de troca (`inventorySaveDatas`, `stashSaveDatas`, `tradingStashSaveDatas`; alias ES3 `remakeTradingStashSaveDatas`) com slots desbloqueados, vazios e itens resolvidos por `itemSaveDatas`; slot ocupado nao deve ser publicado sem `item`: o leitor vivo complementa `itemSaveDatas` com `wk.bezc` quando o slot ja recebeu `ItemUniqueId` antes do commit do `PlayerSaveData.itemSaveDatas`, o resumo usa cache local de `ItemSaveData` por `UniqueId` para resolver movimentos/claims transitorios e a montagem final falha se ainda existir `ItemUniqueId` sem `ItemSaveData`
...
| `GetBoxLog` | Bau obtido; deve virar evento estruturado antes de qualquer regex textual. |
...
- `GetBoxLog` deve ser montado diretamente pela classe IL2CPP antes de qualquer regex textual; mudancas de texto/localizacao nao podem quebrar o gatilho de `Auto-open Chests`.
...
- Para listas de `PlayerSaveData`, prefira `List<TSaveData>` estavel como fallback quando o nome JSON mudar; exemplo: `tradingStashSaveDatas` virou `remakeTradingStashSaveDatas` em 1.00.21, mas continua sendo a lista de slots da aba de troca.
- O cache backend de itens do runtime pode mudar de `List<InventoryItemData>` para `Dictionary<string, InventoryItemData>`; o refresh deve resolver o dono por qualquer uma dessas estruturas e o C++ deve ler os valores dos dois formatos antes de declarar item referenciado como ausente.
